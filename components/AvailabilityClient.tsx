...
const statusOf = (b: BookRow): BookStatus => {
  const req = Number(b.required_qty) || 0;
  const avl = Number(b.available_qty) || 0;
  if (req <= 0) return { type: "noreq", shortBy: 0 };
  if (avl >= req) return { type: "ok", shortBy: 0 };
  return { type: "short", shortBy: req - avl };
};
  ...
  const summary = useMemo(() => {
    ...
    (data?.classes || []).forEach((cls) => {
      (cls.books || []).forEach((b) => {
        ...
        reqTotal += Number(b.required_qty) || 0;
        avlTotal += Number(b.available_qty) || 0;
      });
    });

    return { totalBooks, okCount, shortCount, totalShortQty, reqTotal, avlTotal };
  }, [data]);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-slate-200 shadow-sm">
        <div className="px-4 py-4 flex items-center justify-between gap-3">
          ...
          <div className="text-xs text-slate-600 shrink-0 hidden sm:block">
            {user?.name || "User"}
          </div>
        </div>
          ...
          <div className="grid grid-cols-12 gap-3 items-end">
            ...
            <div className="col-span-12 sm:col-span-6 md:col-span-2">
              ...
              <div className="relative">
                ...
                <input
                  className={`w-full border border-slate-300 rounded-xl pl-10 py-2.5 text-sm bg-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition ${q ? "pr-10" : "pr-4"}`}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              {summary.totalShortQty > 0 && (
                <span className="text-xs px-4 py-1.5 rounded-full bg-rose-200/70 text-rose-900 border border-rose-300 font-bold">
                  Total short: <b>{summary.totalShortQty}</b> books
                </span>
              )}
      ...
      <main className="p-4 max-w-7xl mx-auto">
        ...
        {!loading && !data && (
          <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center text-slate-600 shadow-sm">
            ...
            <div className="text-sm">Select a school and click <b>Check</b> to view book-wise stock status.</div>
          </div>
        )}
            ...
            {filteredAndSortedClasses.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center text-slate-600 shadow-sm">
                {q ? (
                  ...
                  <>
                    ...
                    <div className="text-sm">
                      This school has no book requirements for the selected session.
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="space-y-4">
                {filteredAndSortedClasses.map((cls) => {
                  ...
                  const clsOk = cls.books.filter((b) => statusOf(b).type !== "short").length;
                          ...
                          <table className="w-full text-sm border-collapse">
                            <thead className="bg-slate-100 sticky top-0">
                              <tr>
                                ...
                                <th className="border-b-2 border-slate-300 px-4 py-3 text-left font-bold text-slate-800 w-40">
                                  Status
                                </th>
                              </tr>
                            </thead>
                                ...
                                return (
                                  <tr
                                    ...
                                    className={`border-b border-slate-100 transition hover:bg-slate-50 ${
                                      st.type === "short" ? "bg-rose-50/50" : st.type === "noreq" ? "bg-slate-50/30" : ""
                                    }`}
                                  >
                                    <td className="px-4 py-3">
                                      ...
                                      <div className="text-xs text-slate-500 mt-0.5">
                                        ...
                                        {b.code && (
                                          <span>{b.subject ? " • " : ""}{b.code}</span>
                                        )}
                                      </div>
                                    </td>

                                    <td className="px-4 py-3 text-center font-medium">{b.required_qty}</td>
                                    <td className="px-4 py-3 text-center font-medium">{b.available_qty}</td>
                                    <td className="px-4 py-3 text-center font-medium">{b.reserved_qty}</td>
                                    <td className="px-4 py-3 text-center font-medium">{b.issued_qty}</td>
