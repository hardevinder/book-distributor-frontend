// components/DirectReceiptsPageClient.tsx
"use client";

/**
 * ✅ Direct Receipts (Invoice / Challan)
 * - Same UX as SupplierReceiptsPageClient
 * - NO Order dropdown / NO school_order_id
 * - Supplier optional
 * - School optional
 *
 ...
 * - Create Book modal → POST book → auto-add line + add into dropdown immediately
 */
...
type ListResponse = { receipts: DirectReceipt[] };
type GetResponse = { receipt: DirectReceipt };
...
const statusChip = (s: DirectReceipt["status"]) => {
...
const normalizeDocType = (r: DirectReceipt): ReceiveDocType => {
  const t = String((r as any)?.receive_doc_type || "").toUpperCase();
  if (t === "INVOICE") return "INVOICE";
  if (t === "CHALLAN") return "CHALLAN";
  return (r.invoice_no || r.doc_no) ? "INVOICE" : "CHALLAN";
};

const getDocNo = (r: DirectReceipt) => (r as any)?.doc_no || r.invoice_no || "-";
const getDocDate = (r: DirectReceipt) => (r as any)?.doc_date || r.invoice_date || null;
...
const canEditItems = (r?: DirectReceipt | null) => {
  if (!r) return false;
  if (r.status !== "draft") return false;
  if ((r as any)?.posted_at) return false;
  return true;
};
...
export default function DirectReceiptsPageClient() {
  ...
  const [receipts, setReceipts] = useState<DirectReceipt[]>([]);
  ...
  const [form, setForm] = useState(() => {
    ...
    return {
      ...
      received_date: t,
      status: "draft" as "draft" | "received",
      remarks: "",
  ...
  const cellKey = (rowIdx: number, field: "qty" | "mrp" | "pct" | "amt") => `${rowIdx}-${field}`;
  const focusCell = (rowIdx: number, field: "qty" | "mrp" | "pct" | "amt") => {
  ...
  const onCellEnter = (
    ...
    rowIdx: number,
    field: "qty" | "mrp" | "pct" | "amt"
  ) => {
    if (e.key !== "Enter") return;
    e.preventDefault();
    const order: Array<"qty" | "mrp" | "pct" | "amt"> = ["qty", "mrp", "pct", "amt"];
    const pos = order.indexOf(field);
    if (pos === -1) return;
    if (pos < order.length - 1) focusCell(rowIdx, order[pos + 1]);
    else focusCell(rowIdx + 1, "qty");
  };
  ...
  try {
    ...
    const list: PublisherLite[] = Array.isArray(res.data) ? res.data : res.data?.publishers || res.data?.data || [];
  ...
  const fetchReceipts = async () => {
    ...
    try {
      ...
      const res = await api.get<ListResponse>(API_BASE, { params });
      const list = (res?.data as any)?.receipts;
      setReceipts(Array.isArray(list) ? list : []);
    } catch (e: any) {
      console.error(e);
      setError(e?.response?.data?.error || "Failed to load direct receipts");
    } finally {
      setLoading(false);
    }
  };
 ...
 const title = bookForm.title.trim();
  const publisher_id = Number(bookForm.publisher_id);
  const mrp = Number(bookForm.mrp);

  if (!title) return setBookCreateError("Book title is required.");
  if (!publisher_id) return setBookCreateError("Publisher is required.");
  if (!Number.isFinite(mrp) || mrp <= 0) return setBookCreateError("MRP is required (must be > 0).");
   ...
   const payload: any = {
        title,
        publisher_id,
        mrp, // ✅ required by backend
        class_name: bookForm.class_name?.trim() || null,
        subject: bookForm.subject?.trim() || null,
        code: bookForm.code?.trim() || null,
        isbn: bookForm.isbn?.trim() || null,
      };



      const res = await api.post(BOOKS_API, payload);

      const created: any = (res.data as any)?.book || (res.data as any)?.data || res.data;
      const createdId = Number(created?.id);
      if (!createdId) throw new Error("Book created but id not returned.");

      const newBook: BookLite = {
        id: createdId,
        title: created?.title || title,
        class_name: created?.class_name ?? payload.class_name ?? null,
        subject: created?.subject ?? payload.subject ?? null,
  ...
  const buildPayload = () => {
    ...
    const payload: any = {
      ...
      invoice_date: docType === "INVOICE" ? form.doc_date || undefined : undefined,
  ...
  const submitCreate = async () => {
    ...
    try {
      ...
      if (status === "draft" && payload.receive_doc_type === "CHALLAN") {
        setInfo("Challan saved as DRAFT. Later: Convert to INVOICE (add prices), then Mark Received.");
      } else {
  ...
  const openView = async (id: number) => {
    ...
    try {
      const res = await api.get<GetResponse>(`${API_BASE}/${id}`);
      const row = (res?.data as any)?.receipt as DirectReceipt | undefined;
  ...
  const updateStatus = async (next: "draft" | "received" | "cancelled") => {
    ...
    setError(null);
    try {
      await api.patch(`${API_BASE}/${viewId}/status`, { status: next });
  ...
  const handleViewPdf = async (id: number) => {
    setError(null);
    try {
      const res = await api.get(`${API_BASE}/${id}/pdf`, { responseType: "blob" });
  ...
  const saveConvertAndMaybeReceive = async (markReceived: boolean) => {
    ...
    const docNo = String((convert as any).doc_no || "").trim();
    if (!docNo) {
    ...
    try {
      const patchPayload: any = {
        receive_doc_type: "INVOICE",
        doc_no: docNo,
        doc_date: (convert as any).doc_date || null,
        received_date: (convert as any).received_date || null,
        remarks: String((convert as any).remarks || "").trim() || null,
      ...
      const updated = (res?.data as any)?.receipt as DirectReceipt | undefined;

      if (markReceived) {
        await api.patch(`${API_BASE}/${viewId}/status`, { status: "received" });
      }

      const res2 = await api.get<GetResponse>(`${API_BASE}/${viewId}`);
      const row2 = (res2?.data as any)?.receipt as DirectReceipt | undefined;
  ...
  /* ------------ Listing: client-side doc filters ------------ */
  ...
  const visible = useMemo(() => {
    ...
    return list;
  }, [receipts, filterDocNo, filterDocType]);
  ...
  const selectedSupplierName =
    suppliers.find((s) => String(s.id) === String(form.supplier_id))?.name || (form.supplier_id ? `Supplier #${form.supplier_id}` : "");

  const selectedSchoolName =
    schools.find((s) => String(s.id) === String(form.school_id))?.name || (form.school_id ? `School #${form.school_id}` : "");
      ...
      <header className="sticky top-0 z-20 bg-white border-b border-slate-200">
        <div className="px-3 py-2 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-sm font-semibold truncate">Direct Receipts (Invoice / Challan)</div>
              <div className="text-[11px] text-slate-500 truncate">
                Posting happens only on <b>status = received</b>. Challan can be saved as <b>draft</b> with qty only.
              </div>
            </div>
          </div>
        ...
        <div className="px-3 pb-3 flex flex-wrap items-end gap-2">
          ...
          <div>
            ...
            >
              <option value="">All</option>
              <option value="__NULL__">No Supplier</option>
              {suppliers.map((s) => (
          ...
          {loading ? (
            ...
            <div className="overflow-auto">
              ...
              <div className="px-4 py-2 text-[10px] text-slate-500 border-t">
                ...
                <b>Convert to Invoice</b> and fill prices → Mark <b>Received</b>.
              </div>
            </div>
          )}
        </section>
                ...
                <div className="px-4 py-3 border-b bg-gradient-to-r from-slate-50 to-indigo-50 flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-3 text-[11px] text-slate-700">
                      ...
                      {viewRow?.school?.name || viewRow?.school_id ? (
                        <span className="inline-flex items-center gap-1">
                          <span className="text-slate-500">School:</span>{" "}
                          <span className="font-semibold text-slate-900">
                            {viewRow?.school?.name || `School #${viewRow?.school_id}`}
                          </span>
                        </span>
                      ) : null}
                    ...
                    <>
                      <div className="grid grid-cols-12 gap-3">
                        ...
                        <div className="col-span-12 md:col-span-3 border border-slate-200 rounded-2xl p-3 bg-slate-50">
                          ...
                          <div className="mt-1 text-sm font-extrabold">₹{fmtMoney(viewTotals?.grand_total ?? viewRow.grand_total)}</div>
                        </div>
                      </div>
                  ...
                  <div className="px-4 py-3 border-b bg-gradient-to-r from-slate-50 to-indigo-50 flex items-center justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-slate-600 mt-1 truncate">
                        Receipt: <b>{viewRow.receipt_no || `#${viewRow.id}`}</b> • Works only when <b>Draft</b>.
                      </div>
                    </div>
                    <button
                  ...
                  <div className="p-4">
                    <div className="grid grid-cols-12 gap-3">
                      <div className="col-span-12 md:col-span-4">
                        <MiniLabel>Doc Type</MiniLabel>
                        <select
                          value={(convert as any).receive_doc_type}
                          onChange={(e) => setConvert((p: any) => ({ ...p, receive_doc_type: e.target.value as ReceiveDocType }))}
                      ...
                      <div className="col-span-12 md:col-span-8">
                        ...
                        <input
                          value={(convert as any).doc_no}
                          onChange={(e) => setConvert((p: any) => ({ ...p, doc_no: e.target.value }))}
                          className="w-full border border-indigo-300 rounded-xl px-3 py-2 text-[12px] bg-white"
                      ...
                      <div className="col-span-12 md:col-span-6">
                        ...
                        <input
                          type="date"
                          value={(convert as any).doc_date}
                          onChange={(e) => setConvert((p: any) => ({ ...p, doc_date: e.target.value }))}
                          className="w-full border border-slate-300 rounded-xl px-3 py-2 text-[12px] bg-white"
                        />
                      </div>
                      ...
                      <div className="col-span-12 md:col-span-6">
                        ...
                        <input
                          type="date"
                          value={(convert as any).received_date}
                          onChange={(e) => setConvert((p: any) => ({ ...p, received_date: e.target.value }))}
                          className="w-full border border-slate-300 rounded-xl px-3 py-2 text-[12px] bg-white"
                        />
                      </div>
                      ...
                      <div className="col-span-12">
                        <MiniLabel>Remarks</MiniLabel>
                        <input
                          value={(convert as any).remarks}
                          onChange={(e) => setConvert((p: any) => ({ ...p, remarks: e.target.value }))}
                          className="w-full border border-slate-300 rounded-xl px-3 py-2 text-[12px] bg-white"
                          placeholder="optional"
                        />
                      </div>
                    ...
                    <div className="mt-4 flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        title="Save invoice + prices and mark received"
                      >
                        <CheckCircle2 className="w-4 h-4" />
                    ...
                    <div className="mt-3 text-[11px] text-slate-500">
                      Note: Convert works only when receipt is <b>Draft</b> (not posted). After marking received, items cannot be edited.
                    </div>
                  </div>
                </div>
              </div>
